  ...
  const handleAddData = async (newData) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(`Failed to add member: ${response.status} ${errorDetails}`);
      }
      ...
      <div style={{ display: "flex", justifyContent: "space-between", padding: "10px", alignItems: "center" }}>
        <div>
          ...
          <input
            ...
            style={{ width: "100%", padding: "8px", marginTop: "10px", marginBottom: "10px", border: "1px solid #ccc", borderRadius: "6px", fontSize: "14px" }}
          />
        </div>
        <button
          onClick={() => setIsModalOpen(true)}
          style={{ display: "inline-flex", alignItems: "center", justifyContent: "center", gap: "8px", borderRadius: "6px", fontSize: "14px", fontWeight: "500", backgroundColor: "#000", color: "#fff", padding: "8px 16px", border: "1px", cursor: "pointer", transition: "background-color 0.3s ease" }}
        >
          Add New
        </button>
      </div>
